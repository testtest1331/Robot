...
class RobotContainer:
    ...
    def configureButtonBindings(self) -> None:
        ...
        self._joystick.y().whileTrue(
            # Run the feeder and flywheel together using the subsystem
            commands2.RunCommand(
                lambda: self.shooter.run_flywheel(9.0), 
                self.shooter
            ).alongWith(
                commands2.RunCommand(
                    lambda: self.shooter.run_feeder(0.3), 
                    self.shooter 
                )
            ).finallyDo(
                lambda interrupted: self.shooter.stop()
            )
        )
